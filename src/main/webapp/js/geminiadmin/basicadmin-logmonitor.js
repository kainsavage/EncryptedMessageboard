...
var logmonitor = (function() {
  ...
  wsProcessor = (function() {
    function connect() {
      try {
        ...
        if (svUrl.startsWith("https://")) {
          url = "wss://" + window.location.host + svUrl;
        }
        else {
          url = "ws://" + window.location.host + svUrl;
        }
 
        url = url + "?act=ws&ch=" + channelID;
  ...
  ajaxProcessor = (function() {
    ...
    function fetchViaAjax() {
      $.ajax({
        url: svUrl + "?act=ajax&ch=" + channelID + "&since=" + since,
        dataType: 'json',
        type: 'get',
        data: {},
  ...
  function init(sv, cid, channels) {
    ...
    debug("Attaching listener to log file at " + gba.standardDateString(new Date()) + ".", 50);
    displayChannelInfo();
  ...
  function renderSingleAsHtml(logItem) {
    return e("tr#line" + logItem.uid, [
      e("td.content.l" + logItem.level, [ logItem.text ])
    ]);
  }
  ...
  function repaintLog() {
    ...
    for (i = 0; i < logBuffer.length; i++) {
      if (meetsCriteria(logBuffer[i])) {
        toAdd.push(renderSingleAsHtml(logBuffer[i]));
        dispCurr++;
      }
    }
